...
from schemas import SuccessNote, UpdateSNote
...
async def create_note(note: SuccessNote, db: Session = Depends(get_db), current_user: UserDB = Depends(get_current_user)):
    ...
    if db.query(SuccessDB).filter(SuccessDB.title==new_entry.title,SuccessDB.owner_id == current_user.id).first():
...
async def read_note(note_id: int, db: Session = Depends(get_db),current_user: UserDB = Depends(get_current_user) ):
    db_note=db.query(SuccessDB).filter(SuccessDB.id==note_id).first()
    if db_note is None:
        raise HTTPException(status_code=404, detail="Запись не найдена")
    if db_note.owner_id!=current_user.id:
        raise HTTPException(status_code=403, detail="У текущего пользователя нет такой заметки")
    return db_note
...
async def read_notes_by_page(page:int=1, lim:int = 3, db: Session = Depends(get_db), current_user:UserDB = Depends(get_current_user)):
    # Запрос к базе: "выбрать все записи из таблицы SuccessDB"
    total_count = db.query(SuccessDB).filter(SuccessDB.owner_id==current_user.id).count()
    total_pages = math.ceil(total_count/lim)
    skip = (page-1)*lim

    notes = db.query(SuccessDB).filter(SuccessDB.owner_id==current_user.id).offset(skip).limit(lim).all()
    return {
...
async def delete_note(note_id: int, db: Session = Depends(get_db), current_user:UserDB = Depends(get_current_user)):
    db_note = db.query(SuccessDB).filter(SuccessDB.id==note_id).first()
    if not db_note:
...
async def update_success(note_id: int, note: SuccessNote, db: Session = Depends(get_db), current_user:UserDB = Depends(get_current_user)):
    db_note = db.query(SuccessDB).filter(SuccessDB.id==note_id).first()
    if not db_note:
        raise HTTPException(status_code=400, detail="Такой записи не существует!")
    if db_note.owner_id!=current_user.id:
...
async def newUpdate(id:int, note:UpdateSNote, db:Session = Depends(get_db),current_user: UserDB = Depends(get_current_user)):
...
async def filter_successes(search_query:str, db:Session =Depends(get_db),current_user: UserDB = Depends(get_current_user)):
    notes = db.query(SuccessDB).filter(SuccessDB.title.icontains(search_query), current_user.id==SuccessDB.owner_id).all()
    if not notes:
        return {"status":"OK", "message":f"По ключевому слову '{search_query}' не найдено записей."}
    else:
        return notes
